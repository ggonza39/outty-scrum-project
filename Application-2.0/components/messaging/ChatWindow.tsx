  ...
  useEffect(() => {
    ...
    async function fetchChatDetails() {
      ...
      if (profileRes.data) {
        ...
        const p1 = Array.isArray(conv.participant_1)
          ? conv.participant_1[0]
          : conv.participant_1;
        const p2 = Array.isArray(conv.participant_2)
          ? conv.participant_2[0]
          : conv.participant_2;
    ...
    <div className="flex flex-col h-full min-h-0 bg-black/40">
      ...
      <div className="p-4 border-b border-white/10 flex items-center gap-4 bg-[#022c22]/80 backdrop-blur-md">
        ...
        <div
          ...
          onClick={() =>
            otherUser?.id && router.push(`/profile/${otherUser.id}`)
          }
        >
          {/* THE GLOW RING: Matches the FAB and List style */}
          <div
            className={`absolute inset-0 rounded-full border border-emerald-500 transition-all duration-700 ${
              isOnline && !isLoading
        ...
        >
          ...
          <p
            className={`${isOnline ? "text-emerald-500" : "text-white/50"} text-[13px] font-bold uppercase tracking-tighter transition-colors`}
          >
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar min-h-0">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-full space-y-2">
            ...
            <span className="text-[10px] font-black text-emerald-500/40 uppercase tracking-[0.3em]">
              Decrypting...
            </span>
          </div>
        ) : (
          messages.map((msg) => (
            ...
            >
              <div
                className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm font-medium backdrop-blur-xl border transition-all ${
              ...
              >
                ...
                <div
                  className={`text-[13px] mt-1.5 font-black uppercase tracking-tighter transition-all flex items-center gap-1 ${
                    msg.sender_id === otherUser?.id
                      ? "text-white/30"
                      : "text-emerald-300/50"
                  }`}
                >
                  ...
                  {msg.sender_id !== otherUser?.id && (
                    <span
                      className={`text-[13px] font-bold ${msg.is_read ? "text-emerald-400 opacity-100" : "opacity-40"}`}
                    >
                      {msg.is_read ? "✓✓" : "✓"}
                    </span>
                  )}
      ...
      >
        ...
        <button
          type="submit"
          className="w-12 h-12 bg-emerald-500 rounded-full flex items-center justify-center hover:bg-emerald-400 transition-all active:scale-90 shadow-lg shadow-emerald-500/20"
        >
          <svg
