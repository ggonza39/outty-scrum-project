...
export default function FloatingChatShell() {
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[100] flex flex-col items-end gap-4 pointer-events-none">
      ...
      <div
        className={`chat-container w-[360px] h-[520px] bg-black/40 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.6)] flex flex-col overflow-hidden transition-all duration-300 origin-bottom-right mb-2 ${
          isFloatingOpen
      ...
      >
        ...
        <div className="p-6 relative overflow-hidden flex justify-between items-center border-b border-white/10 bg-emerald-500 shadow-[inset_0_1px_20px_rgba(255,255,255,0.2)]">
          ...
          <div className="flex items-center gap-3 relative z-10">
            {view === "chat" && (
              <button
                ...
                className="p-2 -ml-2 text-[#022c22] hover:bg-black/10 rounded-full transition-all"
              >
                <svg
          ...
          <button
            ...
            className="relative z-10 p-2 text-[#022c22]/60 hover:text-[#022c22] transition-colors"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="22"
        ...
        <div className="flex-1 overflow-y-auto relative bg-white/[0.03] custom-scrollbar">
          ...
          activeFloatingId && isFloatingOpen ? (
            <ChatWindow
              key={activeFloatingId}
              conversationId={activeFloatingId}
            />
          ) : null}
        </div>
      </div>
