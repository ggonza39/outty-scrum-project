...
export default function DashboardFilters({
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 right-0 h-full w-full md:w-96 bg-[#011a14] border-l border-white/10 z-[120] p-8 shadow-2xl transition-transform duration-500 ease-in-out overflow-y-auto custom-scrollbar ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        ...
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={handleReset}
              className="flex items-center gap-1 text-[10px] font-black text-emerald-400/50 hover:text-emerald-400 transition-colors uppercase tracking-widest"
            >
        ...
        <div className="space-y-8 pb-10">
          ...
          <div className="space-y-4 p-4 bg-white/5 border border-white/10 rounded-2xl">
            <div>
              <div className="flex justify-between items-center mb-3">
                <label
                  className={`text-[10px] font-black uppercase tracking-[0.2em] block transition-all duration-300 ${parseInt(draft.radius) >= 5000 ? "nationwide-glow" : "text-emerald-400"}`}
                >
                ...
                <div className="relative flex items-center">
                  ...
                  />
                  <span className="ml-2 text-[10px] font-black text-white/40 uppercase tracking-tighter">
                    Mi
                  </span>
                </div>
              </div>
              ...
              <div className="flex justify-between mt-2 px-1">
                <span className="text-[11px] font-black text-white/40 uppercase">
                  10 mi
                </span>
                <span className="text-[11px] font-black text-white/40 uppercase">
                  5000 mi
            ...
            <div className="pt-2 border-t border-white/5">
              <div className="flex justify-between items-center mb-2">
                <label className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em]">
                  Zip Code
                </label>
                <button
                ...
                >
                  {isLocating && (
                    <div className="w-2 h-2 border-2 border-[#011a14] border-t-transparent rounded-full animate-spin"></div>
                  )}
          ...
          <div className="space-y-4">
            <div>
              <label className="text-[10px] font-black text-emerald-400 uppercase mb-2 block">
                City
              </label>
              <input
                type="text"
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                  {draft.states.map((s: string) => (
                    <span
                      ...
                      className="px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/40 text-emerald-400 text-[8px] font-black rounded flex items-center gap-1 cursor-pointer hover:bg-red-500/10 hover:text-red-400 hover:border-red-400 transition-colors"
                    >
                      {s} <X size={8} />
                    </span>
                  ))}
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl max-h-40 overflow-y-auto p-1 custom-scrollbar">
                {STATE_OPTIONS.map((s) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${isSelected ? "bg-emerald-500 border-emerald-500" : "border-white/20 group-hover:border-emerald-500/50"}`}
                      >
                        {isSelected && (
                          <Check
                            size={12}
                            className="text-[#011a14]"
                            strokeWidth={4}
                          />
                        )}
                      </div>
                      <span
                        className={`text-xs font-bold transition-colors ${isSelected ? "text-white" : "text-white/40"}`}
                      >
                        {s}
                      </span>
                    </div>
          ...
          <div>
            <label className="text-[10px] font-black text-emerald-400 uppercase block mb-3">
              Adventure Type
            </label>
            <div className="flex flex-wrap gap-1.5">
              {ADVENTURE_OPTIONS.map((opt) => (
                <button
                  key={opt}
                  onClick={() => toggleFilter("adventureType", opt)}
                  className={`px-2 py-1.5 rounded-lg text-[9px] font-bold uppercase border transition-all ${draft.adventureType?.includes(opt) ? "bg-emerald-500 border-emerald-500 text-[#011a14]" : "bg-white/5 border-white/10 text-white/60 hover:border-emerald-500/50"}`}
                >
                  {opt}
                </button>
              ))}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {SKILL_OPTIONS.map((opt) => (
                <button
                  key={opt}
                  onClick={() => toggleFilter("skillLevel", opt)}
                  className={`px-2 py-1.5 rounded-lg text-[9px] font-bold uppercase border transition-all ${draft.skillLevel?.includes(opt) ? "bg-emerald-500 border-emerald-500 text-[#011a14]" : "bg-white/5 border-white/10 text-white/60 hover:border-emerald-500/50"}`}
                >
                  {opt}
                </button>
              ))}
            ...
            <div className="flex gap-2">
              {GENDER_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`flex-1 py-1.5 rounded-lg text-[9px] font-bold uppercase border transition-all ${draft.gender?.includes(opt) ? "bg-emerald-500 border-emerald-500 text-[#011a14]" : "bg-white/5 border-white/10 text-white/60 hover:border-emerald-500/50"}`}
                >
                  {opt}
                </button>
              ))}
          ...
          <div>
            <label className="text-[10px] font-black text-emerald-400 uppercase block mb-2">
              Age Range (18+)
            </label>
            ...
            {isAgeInvalid && (
              <p className="text-[9px] text-red-500 font-bold uppercase mt-2 animate-pulse text-center">
