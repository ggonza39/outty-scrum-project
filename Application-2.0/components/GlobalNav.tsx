...
export default function GlobalNav() {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      ...
      const isPublicPage =
        isHomePage || isLoginPage || isOnboardingPage || isAboutPage;
    ...
    <>
      ...
      {isLoggingOut ? (
        ...
        <main className="fixed inset-0 z-[1000] flex items-center justify-center bg-[#022c22] overflow-hidden">
          ...
          <div
            key="exit-greeting"
            className="z-20 text-center px-6 animate-in fade-in zoom-in duration-1000"
          >
        ...
        <>
          ...
          {showToast && (
            <div
              className={`fixed z-[100] transition-all duration-[4500ms] ease-in-out left-1/2 -translate-x-1/2 top-28 md:top-10 ${isExiting ? "opacity-0 blur-2xl scale-90 pointer-events-none" : "opacity-100 blur-0 scale-100 animate-in fade-in zoom-in"}`}
            >
              <div className="bg-[#064e3b]/95 backdrop-blur-3xl border border-emerald-500/30 px-5 py-3 rounded-2xl flex items-center gap-3 text-emerald-50 font-black uppercase tracking-[0.2em] text-[9px] shadow-[0_0_40px_rgba(0,0,0,0.7)] whitespace-nowrap">
                ...
                Adventure Paused •{" "}
                <span className="text-emerald-400">Logged Out</span>
              </div>
            </div>
          )}
              ...
              <div className="w-full flex justify-between items-center pointer-events-auto">
                ...
                {user && !shouldHideGlobalNav && (
                  <nav className="hidden md:flex items-center gap-10 bg-black/40 backdrop-blur-xl border border-white/10 px-8 py-4 rounded-full shadow-2xl">
                    <Link
                      href="/"
                      className="text-white/70 hover:text-emerald-400 font-black uppercase tracking-[0.2em] text-[10px] flex items-center gap-2 transition-all"
                    >
                      <Home size={14} /> Home
                    </Link>
                    <Link
                    ...
                    <Link
                      href="/messages"
                      className="relative text-white/70 hover:text-emerald-400 font-black uppercase tracking-[0.2em] text-[10px] flex items-center gap-2 transition-all"
                    >
                    ...
                    <Link
                      href="/settings"
                      className="group text-white/70 hover:text-emerald-400 font-black uppercase tracking-[0.2em] text-[10px] flex items-center gap-2 transition-all"
                    >
                      <Settings
                        size={14}
