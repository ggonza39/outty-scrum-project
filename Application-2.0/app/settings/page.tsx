...
function SettingsContent() {
  ...
  const toggleSuspension = async () => {
    ...
    if (
      !confirm(isCurrentlyActive ? "Suspend account?" : "Reactivate account?")
    )
      return;
    ...
    <div className="max-w-2xl w-full relative z-20 pt-40 pb-24 px-6">
      ...
      <header className="flex items-center gap-6 mb-12">
        ...
        <div>
          ...
          <p className="text-emerald-400 text-xs font-black uppercase tracking-[0.3em] mt-3 drop-shadow-sm">
      ...
      <div className={styles.glassCard}>
        ...
        <section className="space-y-8 mb-14">
          <div className="flex items-center gap-4 px-2">
            ...
            <h2 className="text-xs font-black uppercase tracking-[0.4em] text-emerald-100/60">
              Privacy & Visibility
            </h2>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-[2rem] p-8 flex justify-between items-center group hover:bg-white/10 transition-all duration-500">
            <div className="max-w-[75%]">
              ...
              <p className="text-sm text-white/50 font-bold leading-relaxed mt-2">
                {isVisible
                  ? "Your profile is active in local adventure feeds."
                  : "Ghost Mode: You are hidden from search results."}
              </p>
            </div>
            <button
            ...
            >
              <div
                className={`absolute top-1 w-6 h-6 bg-white rounded-full shadow-lg transition-all duration-500 ${isVisible ? "left-9" : "left-1"}`}
              />
            </button>
          </div>
        </section>
        ...
        <section className="space-y-8 mb-14">
          ...
          <div className="bg-white/5 border border-white/10 rounded-[2rem] overflow-hidden divide-y divide-white/5">
            ...
            <button
              ...
              className="w-full flex justify-between items-center py-7 px-8 hover:bg-emerald-500/10 transition-all group"
            >
              <span className="font-black text-sm uppercase tracking-widest text-white/80 group-hover:text-emerald-400 transition-colors">
                Reset Password
              </span>
            ...
            <button className="w-full flex justify-between items-center py-7 px-8 opacity-30 cursor-not-allowed">
              <span className="font-black text-sm uppercase tracking-widest text-white/60">
                Two-Factor Auth
              </span>
              <span className="text-[10px] bg-white/10 px-3 py-1.5 rounded-lg text-white/40 font-black uppercase">
                Soon
              </span>
            </button>
          </div>
        ...
        <section className="pt-8 space-y-5">
          ...
          <button
            ...
            className="flex items-center justify-center gap-4 w-full py-6 bg-red-500/10 border border-red-500/30 hover:bg-red-500/20 text-red-500 rounded-[2rem] font-black uppercase text-xs tracking-[0.3em] transition-all duration-300 backdrop-blur-md"
          >
          ...
          <div className="bg-emerald-950/40 border border-white/10 backdrop-blur-[60px] w-full max-w-lg rounded-[3rem] p-12 shadow-2xl animate-in fade-in zoom-in duration-300">
            <div className="flex justify-between items-start mb-10">
              ...
              <button
                ...
                className="p-3 hover:bg-white/10 rounded-full text-white/30"
              >
                <X size={24} />
              </button>
            </div>
            ...
            <p className="text-white/50 text-sm leading-relaxed mb-10 font-bold">
              Purge all data from the Outty grid. This cannot be undone.
            </p>
            ...
            <div className="space-y-8">
              ...
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-5 text-lg font-black tracking-[0.4em] text-center focus:outline-none focus:border-red-500/40 transition-all uppercase text-white placeholder:text-white/10"
              />
              ...
              <button
                ...
                className={`w-full py-6 rounded-2xl font-black uppercase text-xs tracking-[0.3em] transition-all ${deleteConfirmation === "DELETE" ? "bg-red-600 text-white" : "bg-white/5 text-white/10 cursor-not-allowed"}`}
              >
                Finalize Deletion
              </button>
            </div>
      ...
      <div className="fixed inset-0 pointer-events-none z-[5]">
        ...
        {mounted &&
          [...Array(15)].map((_, i) => (
            <div
              key={i}
              className="absolute bg-emerald-400 rounded-full animate-pulse opacity-20"
              style={{
                width: "2px",
                height: "2px",
