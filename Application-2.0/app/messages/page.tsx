...
/* -------------------------------------------------------------------------- */
/* SECTION 2: SUB-COMPONENTS (MessagingContent)                               */
/* -------------------------------------------------------------------------- */
...
function MessagingContent({
  ...
  // Inside MessagingContent in Messages/page.tsx
  useEffect(() => {
  ...
  return (
    <div className="z-10 flex w-full max-w-7xl mx-auto border border-emerald-500/20 overflow-hidden bg-[#022c22]/75 backdrop-blur-lg mb-10 rounded-[2rem] shadow-[0_30px_60px_rgba(0,0,0,0.4)] text-lg md:text-xl h-[750px]">
      ...
      {/* 2.1 Sidebar side */}
      <div className={`flex flex-col border-r border-white/5 text-base md:text-lg flex-none transition-all
      ...
      {/* 2.2 Chat side - SHOW on mobile if ID exists */}
            <div className={`flex-1 flex flex-col bg-white/[0.04] text-lg md:text-xl font-medium min-h-0
              ${!selectedConversationId ? 'hidden md:flex' : 'flex'}`}>
              {selectedConversationId ? (
                <>
                  {/* iOS Style Mobile Back Button */}
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-16 text-center">
            ...
            <h2 className="text-white font-black uppercase tracking-[0.4em] text-2xl mb-4">Outty Inbox</h2>
            <p className="text-white/80 text-base max-w-[360px] leading-relaxed font-bold">
              Select an adventurer from the list to start planning your next trip.
            </p>
          </div>
        )}
      </div>
...
export default function MessagesPage() {
  ...
  return (
    <main className={styles.mainWrapper}>
      ...
      {/* 3.1 Background Layers & Particles */}
      ...
      {/* 3.2 Header Section (Matching Settings Page Styling) */}
     <div className={`z-20 w-full max-w-7xl mx-auto px-8 pt-32 md:pt-40 pb-12 flex-none ${selectedConversationId ? 'hidden md:block' : 'block'}`}>
      ...
      {/* 3.3 Content Layer */}
      <div className={`z-20 w-full max-w-7xl mx-auto flex flex-col transition-all duration-500 ${selectedConversationId ? 'pt-20 md:pt-0' : ''}`}>
        <Suspense fallback={
