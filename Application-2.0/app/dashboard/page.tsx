 ...
 /* -------------------------------------------------------------------------- */
  ...
  const handleStartConversation = async (targetProfile: any) => {
      try {
          ...
          .upsert(
            {
              participant_1: p1,
              participant_2: p2,
              last_message_time: new Date().toISOString() // Using your existing timestamp column
            },
        ...
        if (error) {
          console.error("Upsert Error Details:", error.message, error.details, error.hint);
          return;
        }
      ...
      <div className="flex-grow max-w-6xl mx-auto relative z-10 p-6 md:p-12 pt-48 md:pt-56 w-full">
        ...
        {loading ? (
              ...
              >
                ...
                <div className="flex flex-col flex-grow">
                  ...
                  <div className="py-5 border-t border-white/5 min-h-[80px]">
                    <p className="text-white/40 text-[9px] font-black uppercase tracking-widest mb-3 px-1">Skill Levels</p>
                    <div className="flex flex-wrap gap-2 px-1">
                      {getArrayData(profile.skill_level).length > 0 ? (
                        getArrayData(profile.skill_level).slice(0, 3).map((s: string) => (
                        ...
                        <span className="text-white/20 text-[9px] uppercase italic">Level not set</span>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center bg-white/5 border border-white/5 rounded-3xl backdrop-blur-md">
            ...
            <h3 className="text-xl font-black text-white uppercase tracking-tighter">No Explorers Found</h3>
            <p className="text-white/40 text-xs font-bold uppercase mt-2 max-w-xs leading-relaxed px-4">
              Try expanding your radius or selecting fewer filters to find more partners.
            </p>
            <button
              onClick={() => {
