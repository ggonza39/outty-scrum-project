# Sprint 3 – Daily Scrum Documentation

**Date:** 04-29-2026

---

## 1. What did you do in the last 24 hours to help meet the Sprint Goal?

**Hunter Blake:**  
- Fix photo delete queue insert failure caused by RLS on photo_delete_queue
- Update queue_deleted_photo_cleanup() trigger function to run as SECURITY DEFINER

**Heidi Wilder:**  
- 

**Gibson Garner:**  
- 

**Takeshia Banks:**  
- Fixed US8 gap
- Added delete functionality to ProfilePhotoUploader component
- Implemented delete confirmation modal before removing a photo

**Anthony Nguyen:**  
- Completed a comprehensive visual and technical audit comparing the V1.0 current stubs against V2.0 desired high-fidelity features.
- Drafted the Technical Limitations & Gaps slide to address the messaging handshake and notification constraints.

**Gilberto Gonzalez:**  
- Performed revisions of the project presentation (PPT), ensuring the technical narrative accurately reflects our application's structure and features.
- Provided strategic support to the Product Owner (Anthony) regarding the final presentation flow and demo script to ensure a seamless transition between the slides and the Outty 2.0 live walkthrough.

---

## 2. What will you do in the next 24 hours to help meet the Sprint Goal?

**Hunter Blake:**  
- Continue to support the team on remaining tasks as needed.

**Heidi Wilder:**  
- 

**Gibson Garner:**  
- 

**Takeshia Banks:**  
- Fix any bugs or gaps found during testing

**Anthony Nguyen:**  
- Conduct a final review of all the Sprint 3 acceptance criteria are checked off and that the presentation demo will have the parts needed.
- Continue to manage any new PR so the app is up-to-date and changes are working as intended.

**Gilberto Gonzalez:**  
- Initiate preparations for the Sprint Review and Sprint Retrospective final meetings.
- Begin the final Burndown Chart analysis.
- Conduct a final audit of the Sprint 3 documentation before the May 1st deadline.

---

## 3. Impediments

**Identified Impediment:**  
- None at this time; technical hurdles regarding the real-time sync have been resolved, and focus has shifted to administrative closure.
  
**Impediment Removal Plan:**  
- N/A
  
**Responsible (if applicable):**  
- N/A

---
